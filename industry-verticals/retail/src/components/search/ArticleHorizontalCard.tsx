...
const DEFAULT_IMG_URL = 'https://placehold.co/500x300?text=No Image';
...
const ArticleHorizontalItemCard = ({ className = '', article }: ArticleCardItemCardProps) => {
  let validImageUrl: string | undefined = article.image_url?.trim()
    ? article.image_url
    : DEFAULT_IMG_URL;

  if (validImageUrl && validImageUrl.includes('filters:no_upscale')) {
